...
const char* g_ELL12 = "Thorlabs ELL12";
const char* g_ELL9 = "Thorlabs ELL9";
const char* g_ELL6 = "Thorlabs ELL6";
const char* g_ELL6_shutter = "Thorlabs ELL6 shutter";

// TODO: make an array of value so that the positions can simply be looked up 
const char* g_pos0 = "00000000"; 
const char* g_pos1 = "0000001F"; // decimal value 31
const char* g_pos2 = "0000003E"; // 62
const char* g_pos3 = "0000005D"; // 93
const char* g_pos4 = "0000007C"; // and so on...
const char* g_pos5 = "0000009B";
const char* g_fw = "fw";
const char* g_bw = "bw";
...
MODULE_API void InitializeModuleData()
{
	...
	RegisterDevice(g_ELL12, MM::StateDevice, g_ELL12);
...
MODULE_API MM::Device* CreateDevice(const char* deviceName)
{
	...
	else if (strcmp(deviceName, g_ELL12) == 0) {
		return new ELL12();
	}
	...
	busy_(false)
{
	...
	SetErrorText(ERR_PORT_CHANGE_FORBIDDEN, "Port change is forbidden.");
	SetErrorText(ERR_UNEXPECTED_ANSWER, "The device returned an unexpected answer.");
	SetErrorText(ERR_WRONG_DEVICE, "The device is not an ELL12.");
	SetErrorText(ERR_FORBIDDEN_POSITION_REQUESTED, "Forbidden position requested (allowed: 0, 1, 2, 3, 4 or 5).");
	SetErrorText(ERR_UNKNOWN_STATE, "Unknown state.");
	...
	// Description
	CreateProperty(MM::g_Keyword_Description, "Thorlabs Elliptec 6-position Slider ELL12", MM::String, true);

	// Port
	CPropertyAction* pAct = new CPropertyAction(this, &ELL12::OnPort);
	CreateProperty(MM::g_Keyword_Port, "Undefined", MM::String, false, pAct, true);
	...
	pAct = new CPropertyAction(this, &ELL12::OnChannel);
	CreateProperty("Channel", "0", MM::String, false, pAct, true);
	SetAllowedValues("Channel", channels_vec);
}
...
void ELL12::GetName(char* Name) const
{
	CDeviceUtils::CopyLimitedString(Name, g_ELL12);
}

int ELL12::Initialize()
{
	// ID
	std::string id;
	getID(&id);
	...
	// State
	CPropertyAction* pAct = new CPropertyAction(this, &ELL12::OnState);
...
int ELL12::getID(std::string* id) {
	...
	// check if ELL12 (hex)
	if (message.substr(3, 2).compare("0C") != 0)
		return ERR_WRONG_DEVICE;
...
int ELL12::getState(int* state) {
	...
	// if the positions were stored in an array, this whole block could be simplified
	std::string position = removeCommandFlag(message);
	...
	else if (position.compare(g_pos4) == 0) {
		*state = 4;
	}
	else if (position.compare(g_pos5) == 0) {
		*state = 5;
	}
	else {
...
//---------------------------------------------------------------------------
std::string ELL12::removeLineFeed(std::string answer) {
	std::string message;
	if (answer.substr(0, 1).compare("\n") == 0) {
		message = answer.substr(1, answer.length() - 1);
	}
...
std::string ELL12::removeCommandFlag(std::string message) {
	std::string value = message.substr(3, message.length() - 3);
	return value;
}

bool ELL12::isError(std::string message) {
	if (message.substr(1, 2).compare("GS") == 0) {
		return true;
	}
	return false;
...
int ELL12::getErrorCode(std::string message) {
	std::string code = removeCommandFlag(message);
...
int ELL12::OnState(MM::PropertyBase* pProp, MM::ActionType eAct) {
	if (eAct == MM::BeforeGet) {
		int state;
...
int ELL12::OnPort(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	if (eAct == MM::BeforeGet)
	{
		pProp->Set(port_.c_str());
...
int ELL12::OnChannel(MM::PropertyBase* pProp, MM::ActionType eAct)
{
	if (eAct == MM::AfterSet)
	{
		std::string channel;
